...
func createInteractiveHelp(userID int64, helpEmbeds []*discordgo.MessageEmbed) (interface{}, error) {
	...
	if err != nil {
		return "Something went wrong, maybe you have DMs disabled? I don't want to spam this channel so here's a external link to available commands: <https://docs.sgpdb.xyz/commands>", err
	}

	// prepend a introductionairy first page
	firstPage := &discordgo.MessageEmbed{
		Title: "SGPDB Help!",
		Description: fmt.Sprintf(`SGPDB is an open-source multipurpose discord bot that is configured through the web interface at %s.
For more in depth help and information you should visit https://docs.sgpdb.xyz/ as this command only shows information about commands.)
