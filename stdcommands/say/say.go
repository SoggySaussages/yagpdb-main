...
var Command = &commands.YAGCommand{
	...
	Name:                "Say",
	Aliases:             []string{"repeat", "parrot"},
	Description:         "Repeat the text as a Webhook\nYou can edit existing messages by supplying the `-message` flag.",
	...
	RunFunc: func(data *dcmd.Data) (interface{}, error) {
		...
		var params = &discordgo.WebhookParams{
			Username:  common.BotUser.Username,
			AvatarURL: common.BotUser.Avatar,
		}
		...
		if name, set := getSwitch("author"); set {
			params.Username = name.(string)
			modifiedEmbed = true
		}
		if icon, set := getSwitch("authoricon"); set {
			params.AvatarURL = icon.(string)
			modifiedEmbed = true
		}
		...
		webhook, err := common.BotSession.FirstAvailableWebhook(cID, common.BotUser.ID)
		if err != nil {
			return err, err
		}
		...
		if msg == nil {
			send := params
			params.AllowedMentions = &discordgo.AllowedMentions{}
			...
			_, err := common.BotSession.WebhookExecuteComplex(webhook.ID, webhook.Token, true, send)
			if err != nil {
				return err, err
			}
		...
		_, err = common.BotSession.WebhookMessageEdit(webhook.ID, discordgo.StrID(mID), webhook.Token, edit)
		if err != nil {
			return err, err
		}
		return "Done", nil
