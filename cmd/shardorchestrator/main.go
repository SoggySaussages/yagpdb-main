...
import (
	...
	"github.com/SoggySaussages/syzygy/lib/dshardorchestrator/orchestrator"
	"github.com/SoggySaussages/syzygy/lib/dshardorchestrator/orchestrator/rest"
	"github.com/mediocregopher/radix/v3"
	"github.com/sirupsen/logrus"

	_ "github.com/SoggySaussages/syzygy/bot" // register the custom orchestrator events
)
...
func main() {
	...
	if totalShards < 1 {
		panic("SYZYGY_SHARDING_TOTAL_SHARDS needs to be set to a resonable number of total shards")
	}

	if len(activeShards) < 1 {
		panic("SYZYGY_SHARDING_ACTIVE_SHARDS is not set, needs to be set to the shards that should be active on this host, ex: '1-49,60-99'")
	}
	...
	orch.NodeLauncher = &orchestrator.StdNodeLauncher{
		...
		LaunchArgs:    []string{"./syzygy", "-bot", "-syslog"},

		VersionCmdName: "./syzygy",
	...
	restAPIAddr := os.Getenv("SYZYGY_BOTREST_LISTEN_ADDRESS")
	if restAPIAddr == "" {
