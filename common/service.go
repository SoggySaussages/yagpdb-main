...
func (s *serviceTracker) update() {
	...
	err = RedisPool.Do(redis.FlatCmd(nil, "ZADD", ServicesRedisKey, time.Now().Unix(), serialized))
	if err != nil {
	...
	if !bytes.Equal(serialized, s.lastUpdate) {
		err = RedisPool.Do(redis.FlatCmd(nil, "ZREM", ServicesRedisKey, s.lastUpdate))
		if err != nil {
	...
	err = RedisPool.Do(redis.FlatCmd(nil, "ZREMRANGEBYSCORE", ServicesRedisKey, 0, time.Now().Unix()-30))
	if err != nil {
...
func (sp *servicePoller) getActiveServiceHosts() ([]*ServiceHost, error) {
	...
	err := RedisPool.Do(redis.FlatCmd(&hosts, "ZRANGE", ServicesRedisKey, 0, -1))
	if err != nil {
